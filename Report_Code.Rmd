...
<br>
<image src="screenshot/picture4.jpg" alt="picture1" width="880" height="500" align="middle">
<br>
...
metrosubset<-filter(metro,year %in% c(2006,2007,2008,2009,2010,2011,2012,2013,2014,2015,2016), quarter==4)
metro2<-dcast(metrosubset,name~year+quarter,value.var="HPI")
pander::pander(head(metro2))
```


#Summary of House Price Index(Purchase Only Index) 
...
```{r,warning=FALSE,message=FALSE}
#Get the average  Index for Each Year for all states (Average over 4 Quarters)
...
ggparcoord(df.states2, columns = c(2:(length(df.states2)-3)), groupColumn = 25, scale = "globalminmax",
           missing = "exclude") + theme(legend.position = "right", text=element_text(size=14),legend.text = element_text(size =10),axis.text.x = element_text(size=8,angle=-45, vjust=1))+xlab("Year")+ylab("House Price Index(1991=100)")+ggtitle("Average HPI for each year by US Regional Class")
```
...
I then analyzed percentage change of HPI (Purchase Only Index) over previous quarter, previous year, past 5 years and since the first quarter of 1991. D.C and USA result is also included in the analysis. Period ended is 2016-Q3. In dataset US-HPI, since the region "USA" has leading and trailing whitespaces, we need to trim it off to combine US HPI data with US map data. 
...
mutcomb.division<-Change_HPI(comb.division)
...
```{r,warning=FALSE,message=FALSE}
mutcomb.state<-Change_HPI(comb.state)
...
plotChange_HPI<-function(subset,title){
  colnames(subset)<-c("region","value")
  midvalue=filter(subset,region=="usa")$value  #USE USA value as midvalue
  mapdf2<-merge(subset,mapdf,by="region")
  ggplot(mapdf2) + 
...
```{r,warning=FALSE,message=FALSE,fig.width=14,fig.height=10,fig.align='center'}
mapdf <- map_data("state")
...
<br>
<image src="screenshot/picture2.jpg" alt="picture1" width="880" height="500" align="middle">
<br>

<br>
<image src="screenshot/picture3.jpg" alt="picture1" width="880" height="500" align="middle">
<br>
...
<p id="caption">5.Discussion and future work </p>


More research to further investigate the statistical properties of repeat transactions house price indexes is needed. Many of these issues are discussed in the paper by Stephens et al. (1996). Topics for future research include: geographic and temporal aggregation, revision volatility, the use of appraisal values, sample selection, and comparisons with alternative methods.
